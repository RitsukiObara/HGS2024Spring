...
// 設定処理
//===========================================
void CAnimReaction::SetData(const D3DXVECTOR3& pos, const D3DXVECTOR3& size, const D3DXCOLOR& col, const TYPE type, const int nCount, const int nRepeat)
{
	// スクロールの設定処理
	SetPos(pos);				// 位置設定
	SetSize(size);				// サイズ設定
	...
	// 全ての値を設定する
	m_type = type;				// 種類
	m_nLife = nCount * ANIM_PATTERN[type] * nRepeat;		// 繰り返す回数
...
// 生成処理
//===========================================
CAnimReaction* CAnimReaction::Create(const D3DXVECTOR3& pos, const D3DXVECTOR3& size, const D3DXCOLOR& col, const TYPE type, const int nCount, const int nRepeat)
{
	// ローカルオブジェクトを生成
	...
	{ // オブジェクトが NULL じゃない場合
	  ...
	  // 初期化処理
		...
		// 情報の設定処理
		pReaction->SetData(pos, size, col, type, nCount, nRepeat);
	}
	else
	{ // オブジェクトが NULL の場合
